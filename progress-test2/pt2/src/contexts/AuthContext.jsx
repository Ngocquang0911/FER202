...
export const AuthProvider = ({ children }) => {
  ...
  const login = async ({ usernameOrEmail, password }) => {
    ...
    try {
      ...
      if (user) {
        // Chỉ cho phép login nếu là admin và trạng thái active
        if (user.role !== 'admin') {
          const errorMessage = 'Access denied: you must be an admin to access the dashboard.';
          dispatch({ type: 'LOGIN_FAILURE', payload: errorMessage });
          return { success: false, error: errorMessage };
        }

        if (user.status !== 'active') {
          const errorMessage = 'Tài khoản bị khóa hoặc không hoạt động. Bạn không có quyền truy cập.';
          dispatch({ type: 'LOGIN_FAILURE', payload: errorMessage });
          return { success: false, error: errorMessage };
        }
